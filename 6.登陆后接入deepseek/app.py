...
def index(): #函数名称，可自定义，为方便记忆可以与路由名称一致
    return render_template('openai.html') #这是flask访问静态页面的方法，即执行index()函数即打开index.html静态网页，注意index.html一定要放在templates目录下，其他静态内容如js、css、图片等放在static目录下。
